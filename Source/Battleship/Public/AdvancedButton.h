...
*/
class BATTLESHIP_API SAdvancedButton : public SButton {
public:
	UTexture2D* AdvancedHitTexture;
	int AdvancedHitAlpha;

	SAdvancedButton() : AdvancedHitTexture(NULL), AdvancedHitAlpha(0) {}
 ...
 */
UCLASS()
class BATTLESHIP_API UAdvancedButton : public UButton
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "AdvancedHitTest") void SetAdvancedHitTexture(UTexture2D* InTexture) {
		...
		if (MyButton.IsValid()) (static_cast<SAdvancedButton*>(MyButton.Get()))->SetAdvancedHitTexture(AdvancedHitTexture);
	}
