...
FMargin SDialogueNodeWidget::GetTextMargin() const
{
	...
	if (Owner->GetZoomLevel() > 6 && (Dialogue->Data[NodeIndex].bHasEvents || Dialogue->Data[NodeIndex].bHasConditions || Dialogue->Data[NodeIndex].Sound || Dialogue->Data[NodeIndex].DialogueWave))
	{
...
EVisibility SDialogueNodeWidget::GetEventIconVisibility() const
{
	...
	return Dialogue->Data[NodeIndex].bHasEvents ? EVisibility::SelfHitTestInvisible : EVisibility::Collapsed;
}
...
EVisibility SDialogueNodeWidget::GetConditionIconVisibility() const
{
	...
	return Dialogue->Data[NodeIndex].bHasConditions ? EVisibility::SelfHitTestInvisible : EVisibility::Collapsed;
}
...
FReply SDialogueNodeWidget::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	else if (MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		...
		FUIAction DeleteAllAction(FExecuteAction::CreateSP(Owner, &SDialogueViewportWidget::DeleteAllSelected));
		...
		{
			MenuBuilder.AddMenuEntry(NSLOCTEXT("PropertyView", "DeleteAll", "Delete Selected Nodes"), NSLOCTEXT("PropertyView", "DeleteAll_ToolTip", "Deletes all selected nodes"), FSlateIcon(), DeleteAllAction);
		}
